...
public class AdminNormalOrderController {
	...
	private boolean validateInputForFinishingForm(RedirectAttributes attr, String cost, String discount) {
		boolean result = true;
		if(!cost.isEmpty()) {
			try {
	...
	private boolean addFlashAttribute(RedirectAttributes attr, String data, String about) {
	...
	public String backToFinishedOrdersList(HttpSession session, Model model) throws ParseException {
		...
		System.out.println(first_date_finished_orders);
		System.out.println(second_date_finished_orders+"---------------");
